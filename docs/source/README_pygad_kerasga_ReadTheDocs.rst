...
2. Prepare the Keras model weights into a vector using the
   ``pygad.kerasga.model_weights_as_vector()`` function.
...
6. Build the generation callback function.

7. Create an instance of the ``pygad.GA`` class.
...
used as instance attributes in addition to adding a new attribute.
...
``pygad.gacnn.model_weights_as_vector()`` 
------------------------------------------
...
``pygad.gacnn.model_weights_as_matrix()``
-----------------------------------------
...
Prepare the Keras Model Weights into a Vector
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The second step is to represent the model weights as a vector. This is
by passing the model to the ``pygad.kerasga.model_weights_as_vector()``
function.

.. code:: python

   weights_vector = kerasga.model_weights_as_vector(model=model)
...
The third step is to create an instance of the ``pygad.kerasga.KerasGA``
class. There are 10 solutions per population. Change this number
...
The fourth step is to prepare the training data inputs and outputs. Here
...
The fifth step is to build the fitness function. This function must
...
Build the Generation Callback Function
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The sixth step is to build a function that is called after each
generation. This function updates the population according to the
solutions from the last generation. It just accepts a single parameter
representing the instance of the ``pygad.GA`` class.
...
The seventh step is to instantiate the ``pygad.GA`` class. For more
information, please check the `parameters this class
...
The eighth and last step is to run the genetic algorithm by calling the
``run()`` method.
...
-  The trained data is changed. Note that the output of each sample is a
...
**Reading the Data** section of the ``pygad.cnn`` module.
